...
export default class extends Store {
  ...
  queryRecord(modelName, query) {
    ...
    return this.query(modelName, query).then(result => new RSVP.Promise((resolve) => resolve(result.firstObject)));
  }
  ...
  findRecord(modelName, id, options) {
    ...
    if (options && options.projection) {
      ...
      builder.selectByProjection(options.projection);
      return this.query(modelName, builder.build()).then(result => new RSVP.Promise((resolve) => resolve(result.firstObject)));
    }
    ...
    // Now if projection is not specified then only 'id' field will be selected.
    queryObject.select = [];
    return this.query(modelName, queryObject).then(result => new RSVP.Promise((resolve) => resolve(result.firstObject)));
  }
