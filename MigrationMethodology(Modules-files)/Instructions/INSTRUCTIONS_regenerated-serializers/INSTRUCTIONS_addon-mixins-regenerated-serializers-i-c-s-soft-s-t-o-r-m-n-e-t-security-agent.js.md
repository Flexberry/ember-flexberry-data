# Миграция: regenerated-serializers/i-c-s-soft-s-t-o-r-m-n-e-t-security-agent.js
...
export default ODataSerializer.extend({
  // Логика для i-c-s-soft-s-t-o-r-m-n-e-t-security-agent.js
});
```

### 2. Целевой файл (Next.js 16)
- `app/lib/transforms/serializer-$(echo i-c-s-soft-s-t-o-r-m-n-e-t-security-agent.js | cut -d. -f1).ts` — function
...
```typescript
// app/lib/transforms/serializer-$(echo i-c-s-soft-s-t-o-r-m-n-e-t-security-agent.js | cut -d. -f1).ts
import { transformOdataResponse } from '@/lib/transforms/serializer-odata';

export function transformi-c-s-soft-s-t-o-r-m-n-e-t-security-agentResponse<T = any>(data: any): T {
