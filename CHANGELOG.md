...
- Bower dependencies removed and replaced with npm packages, so `bower install` is unnecessary now.
...
- Deprecated copy functions from `@ember/object/internals` replaced with functions from `ember-copy`.
- Deprecated `Ember.Map` replaced with native `Map` class.

### Known issues
- Single module test `Unit | Model | model: rollback hasMany relationships` is failing after update, because model's `canonicalState` is `undefined`.
