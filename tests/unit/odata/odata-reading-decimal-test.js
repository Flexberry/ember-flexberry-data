...
function initTestData(store) {
    return Ember.RSVP.Promise.all([
        store.createRecord('ember-flexberry-dummy-decimal-number', {
            decimalNumber: 555.5
        }).save(),
...
executeTest('reading | decimal', (store, assert) => {
    ...
    Ember.run(() => {
        initTestData(store)
            ...
            .then(() => {
                let builder = new QueryBuilder(store, 'ember-flexberry-dummy-decimal-number')
                    .where('decimalNumber', FilterOperator.Eq, 555.5);
                return store.query('ember-flexberry-dummy-decimal-number', builder.build())
                    .then((data) => {
